...
/// <summary>
/// Test that the date saver app will throw error on invalid date format.
/// </summary>

TEST(AddInvalidDateTest, GivenInvalidDateWhenAddExpectFalse) {
    MockTimeKeeper keeper;

    EXPECT_CALL(keeper, GetCurrentDate)
        .WillOnce(Return(""))
        .WillOnce(Return("Feb-21"));

    DateSaverApp saverApp(&keeper);

    EXPECT_THROW(saverApp.AddNewDate(), std::invalid_argument);
    EXPECT_THROW(saverApp.AddNewDate(), std::invalid_argument);

}

/// <summary>
/// Test that the date saver app will correctly add current date data.
/// </summary>

TEST(AddValidDateTest, GivenEmptyDatesWhenAddExpectTrue) {
    MockTimeKeeper keeper;

    EXPECT_CALL(keeper, GetCurrentDate)
        .WillOnce(Return("02-21"));

    DateSaverApp saverApp(&keeper);

    EXPECT_TRUE(saverApp.AddNewDate());

}

/// <summary>
/// Test that false is returned if date is already stored in db.
/// </summary>

TEST(AddContainedDateTest, GivenDateContainedWhenAddExpectFalse) {
    MockTimeKeeper keeper;

    string setDate = "02-21";
    EXPECT_CALL(keeper, GetCurrentDate)
        .WillOnce(Return(setDate))
        .WillOnce(Return(setDate));

    DateSaverApp saverApp(&keeper);

    EXPECT_TRUE(saverApp.AddNewDate());
    EXPECT_FALSE(saverApp.AddNewDate());
...
/// <summary>
/// Test that the date saver app will correctly identify if date is not found.
/// </summary>

TEST(DateNotFoundTest, GivenDateNotContainedWhenCheckedExpectNotFoundMessage) {
    MockTimeKeeper keeper;
    ...
    EXPECT_EQ(saverApp.VerifyDate(), "Date is not significant.");
}

/// <summary>
/// Test that the date saver app will validate significance of date if found.
/// </summary>

TEST(DateFoundTest, GivenDateContainedWhenCheckedExpectFoundMessage) {
    MockTimeKeeper keeper;
    ...
    EXPECT_TRUE(saverApp.AddNewDate());
    EXPECT_EQ(saverApp.VerifyDate(), "This Date is significant!");
}
